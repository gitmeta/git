...
class Stage {
    ...
    func add(_ file: URL, index: Index) throws {
        guard file.path.contains(repository.url.path) else { throw Failure.Add.outside }
        ...
        let folder = repository.url.appendingPathComponent(".git/objects/\(hash.1.prefix(2))")
